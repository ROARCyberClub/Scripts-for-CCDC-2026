...
# PURPOSE: Configure ufw with web server ports and security rules
...
for proto in "${ALLOWED_PROTOCOLS[@]}"; do
    case "$proto" in
        "ssh") ;;
        "http")
            run_ufw_cmd "allow 80/tcp comment 'HTTP'"
            success "HTTP (80) allowed"
            ;;
        "https")
            run_ufw_cmd "allow 443/tcp comment 'HTTPS'"
            success "HTTPS (443) allowed"
            ;;
        "mysql")
            run_ufw_cmd "allow 3306/tcp comment 'MySQL'"
            success "MySQL (3306) allowed"
            ;;
        "icmp")
            # UFW allows ICMP by default
...
info "Web Ports: 80 (HTTP), 443 (HTTPS)"
info "Database: 3306 (MySQL)"
