#!/bin/bash
# ==============================================================================
# SCRIPT: rollback.sh (Configuration Rollback - ufw)
...
rollback_firewall() {
    local backup_path="$1"
    subheader "Firewall Rollback (ufw)"
    
    if [ -f "${backup_path}/user.rules" ]; then
        restore_ufw "$backup_path"
        success "Firewall rules restored"
    else
        warn "No ufw backup found"
    fi
}
...
rollback_config() {
    ...
    if confirm "Restart web services?" "y"; then
