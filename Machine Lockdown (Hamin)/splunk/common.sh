...
backup_firewalld() {
    ...
    # Export current permanent configuration
    firewall-cmd --list-all --permanent > "${rollback_path}/firewalld.conf" 2>/dev/null
    firewall-cmd --list-all-zones --permanent > "${rollback_path}/firewalld-zones.conf" 2>/dev/null
...
restore_firewalld() {
    ...
    # Read and re-add ports
    if [ -f "${rollback_path}/firewalld-ports.txt" ]; then
        # First remove all current ports
    ...
    # Read and re-add services
    if [ -f "${rollback_path}/firewalld-services.txt" ]; then
        while read -r service; do
            [ -n "$service" ] && firewall-cmd --permanent --add-service="$service" 2>/dev/null
        done < "${rollback_path}/firewalld-services.txt"
    fi
    
    # Read and re-add rich rules
    if [ -f "${rollback_path}/firewalld-rich-rules.txt" ]; then
        while read -r rule; do
            [ -n "$rule" ] && firewall-cmd --permanent --add-rich-rule="$rule" 2>/dev/null
        done < "${rollback_path}/firewalld-rich-rules.txt"
    fi
