...
# ------------------------------------------------------------------------------
# 7.5. UFW SUPPORT (Ubuntu 24)
# ------------------------------------------------------------------------------

# Save ufw rules for rollback
backup_ufw() {
    local rollback_path="$1"
    
    if is_dry_run; then
        info "[DRY-RUN] Would backup ufw rules"
        return 0
    fi
    ...
    ufw status verbose > "${rollback_path}/ufw-status.txt" 2>/dev/null
    cp /etc/ufw/user.rules "${rollback_path}/user.rules" 2>/dev/null
    cp /etc/ufw/user6.rules "${rollback_path}/user6.rules" 2>/dev/null
    
    info "Backed up ufw rules to: $rollback_path"
}
...
restore_ufw() {
    ...
    if [ -f "${rollback_path}/user.rules" ]; then
        cp "${rollback_path}/user.rules" /etc/ufw/user.rules 2>/dev/null
        cp "${rollback_path}/user6.rules" /etc/ufw/user6.rules 2>/dev/null
        ufw reload 2>/dev/null
        info "Restored ufw from: $rollback_path"
    fi
}

# Run ufw command with dry-run support
run_ufw_cmd() {
    local cmd="$1"
    
    if [ "$DRY_RUN" == "true" ]; then
        echo -e "${CYAN}[DRY-RUN]${NC} Would execute: ufw $cmd"
        log_info "[DRY-RUN] ufw $cmd"
        return 0
    else
        log_action "Executing: ufw $cmd"
        eval "ufw $cmd"
        return $?
    fi
}
